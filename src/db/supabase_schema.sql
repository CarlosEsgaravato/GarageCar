...
-- SCRIPT DE INICIALIZAÇÃO DE BANCO DE DADOS POSTGRESQL / SUPABASE
-- FASE 1: FUNDAÇÃO, BANCO, SEGURANÇA (RLS), STORAGE E AUDITORIA
-- ====================================================================
...
-- 2.1 TABELA: CLIENTES
-- Regra: NUNCA criar CPF. Um cliente pode possuir vários veículos.
...
-- 2.2 TABELA: VEÍCULOS
-- Regra: Categoria comercial livremente editável, placa opcional.
CREATE TABLE IF NOT EXISTS public.vehicles (
    ...
    commercial_category VARCHAR(50) NOT NULL CHECK (
        commercial_category IN ('Carro / Compacto', 'SUV / Crossover', 'Porte Médio / Pickup', 'Moto')
    ),
    notes TEXT DEFAULT '',
    is_active BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- 2.3 TABELA: CATÁLOGO DE SERVIÇOS
-- Regra: Preços devem usar NUMERIC(10,2). Histórico de serviço nunca é alterado por edição aqui.
CREATE TABLE IF NOT EXISTS public.service_catalog (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(255) NOT NULL,
    vehicle_type VARCHAR(20) NOT NULL CHECK (vehicle_type IN ('car', 'motorcycle')),
    commercial_category VARCHAR(50) NOT NULL,
...
-- 2.4 TABELA: CONFIGURAÇÃO DE ACRÉSCIMOS DE CONDIÇÃO
...
-- 2.5 TABELA: PRODUTOS
-- Regra: Para líquidos utilizar 'ml' como unidade interna (ex: 3L = 3000ml).
...
-- 2.6 TABELA: LOTES DE PRODUTOS
-- Regra: Preserva data de compra, quantidade, valor total, custo unitário, fornecedor e origem do recurso.
-- Nunca substituir o preço histórico de um lote.
...
-- 2.7 TABELA: MOVIMENTAÇÕES DE ESTOQUE
-- Regra: Entrada por compra, consumo por serviço, ajuste positivo, ajuste negativo.
-- Estoque negativo é permitido. Ajuste manual com motivo obrigatório.
...
-- 2.8 TABELA: AGENDA (AGENDAMENTOS)
...
-- 2.9 TABELA: SERVIÇOS REALIZADOS (EXECUTADOS)
-- Regra: Snapshots históricos imutáveis preservados. Receita/hora calculada.
-- Pagamento não parcial: Pendente, Pago, Cancelado.
...
-- 2.10 TABELA: PRODUTOS UTILIZADOS NO SERVIÇO EXECUTADO (CUSTO HISTÓRICO FIFO)
-- Regra: Cada serviço armazena custo unitário no momento do uso e custo total imutável.
...
-- 2.11 TABELA: MOVIMENTAÇÕES FINANCEIRAS
-- Regra: Separar receitas, despesas, aportes, caixa. Correções nunca apagam registro original.
CREATE TABLE IF NOT EXISTS public.financial_transactions (
    ...
    executed_service_id UUID REFERENCES public.executed_services(id) ON DELETE SET NULL,
    is_reversed BOOLEAN NOT NULL DEFAULT false,
    reversal_reason TEXT DEFAULT '',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- 2.12 TABELA: APORTES DE CAPITAL
-- Regra: Diferenciar 1. dinheiro na empresa vs 2. compra paga pelo proprietário.
-- Aportes NÃO são receita e não reduzem o caixa quando pagos pelo proprietário.
...
-- 2.15 TABELA: CONFIGURAÇÕES DO SISTEMA
...
-- 2.16 TABELA: AUDITORIA DO SISTEMA
-- Regra: Registrar usuário, ação, entidade, ID, estado anterior, novo estado, data/hora.
...
-- Regra: Ativar RLS em TODAS as tabelas. Nenhum dado administrativo
...
-- POLÍTICAS: Acesso restrito a usuários autenticados (Administrador)
DO $$
DECLARE
    tbl text;
    tables text[] := ARRAY[
        'clients', 'vehicles', 'service_catalog', 'condition_surcharges',
        'products', 'product_batches', 'stock_movements', 'appointments',
        'executed_services', 'executed_service_products', 'financial_transactions',
        'capital_contributions', 'equipment', 'wishlist', 'system_settings',
        'audit_logs'
    ];
BEGIN
...
-- Regra: Armazenamento da logo da Garage Car e futuros anexos.
-- ====================================================================
...
-- ====================================================================
-- 5. FUNÇÃO TRANSACIONADA RPC: FINALIZAÇÃO ATÔMICA DE SERVIÇO
-- Mantém consistência estrita entre serviço, consumo de lotes (FIFO),
-- movimentação de estoque, histórico e transação financeira quando pago.
-- ====================================================================
...
-- 5.2 FUNÇÃO RPC TRANSACIONADA: REGISTRO DE COMPRA / ENTRADA DE LOTE
...
-- ====================================================================
-- 6. DADOS INICIAIS (SEED) - REGRAS ESTRITAS DAS SEÇÕES 6, 7, 8, 9, 10
-- ====================================================================

-- 6.1 ACRÉSCIMOS DE CONDIÇÃO (Seção 10)
INSERT INTO public.condition_surcharges (condition_level, label, suggested_amount)
VALUES 
    ('normal', 'Normal', 0.00),
...
-- 6.2 CONFIGURAÇÕES INICIAIS DA GARAGE CAR (Seção 11)
...
-- 6.3 SERVIÇOS PRINCIPAIS E PREÇOS INICIAIS (Seções 7, 8 e 9)

-- Carro / Compacto
INSERT INTO public.service_catalog (name, vehicle_type, commercial_category, service_type, base_price, description, included_items)
VALUES 
(
    'Lavagem Convencional', 'car', 'Carro / Compacto', 'convencional', 70.00,
...
(
    'Lavagem Técnica', 'car', 'Carro / Compacto', 'tecnica', 109.90,
...
(
    'Lavagem Premium', 'car', 'Carro / Compacto', 'premium', 199.90,
...
VALUES 
(
    'Lavagem Convencional', 'car', 'SUV / Crossover', 'convencional', 90.00,
    'Lavagem padrão externa e interna para SUVs e Crossovers.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus"]'::jsonb
),
(
    'Lavagem Técnica', 'car', 'SUV / Crossover', 'tecnica', 129.90,
    'Lavagem detalhada com proteção de cera por até 3 meses para SUVs.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus", "Proteção da pintura com cera (até 3 meses)", "Condicionamento dos plásticos internos", "Selante para pneus com maior resistência à água"]'::jsonb
),
(
    'Lavagem Premium', 'car', 'SUV / Crossover', 'premium', 209.90,
    'Detalhamento premium completo para SUVs e Crossovers.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus", "Proteção da pintura com cera (até 3 meses)", "Condicionamento dos plásticos internos", "Selante para pneus com maior resistência à água", "Limpeza e hidratação de bancos de couro", "Revitalização dos plásticos externos"]'::jsonb
);
...
VALUES 
(
    'Lavagem Convencional', 'car', 'Porte Médio / Pickup', 'convencional', 100.00,
    'Lavagem padrão para pick-ups e veículos de porte médio.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus"]'::jsonb
),
(
    'Lavagem Técnica', 'car', 'Porte Médio / Pickup', 'tecnica', 139.90,
    'Lavagem detalhada técnica para pick-ups e veículos médios.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus", "Proteção da pintura com cera (até 3 meses)", "Condicionamento dos plásticos internos", "Selante para pneus com maior resistência à água"]'::jsonb
),
(
    'Lavagem Premium', 'car', 'Porte Médio / Pickup', 'premium', 219.90,
    'Detalhamento premium e hidratação para pick-ups e médios.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza das rodas", "Secagem com microfibra", "Aspiração interna", "Limpeza básica de painel, console e portas", "Limpeza dos vidros", "Pretinho para pneus", "Proteção da pintura com cera (até 3 meses)", "Condicionamento dos plásticos internos", "Selante para pneus com maior resistência à água", "Limpeza e hidratação de bancos de couro", "Revitalização dos plásticos externos"]'::jsonb
);
...
VALUES 
(
    'Lavagem Convencional', 'motorcycle', 'Moto', 'convencional', 35.00,
    'Lavagem externa padrão para motocicletas.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza detalhada das rodas e kit relação", "Secagem", "Pretinho para pneus"]'::jsonb
),
(
    'Lavagem Técnica', 'motorcycle', 'Moto', 'tecnica', 70.00,
    'Lavagem detalhada com proteção de cera e selante protetor para motos.',
    '["Pré-lavagem", "Shampoo pH neutro", "Limpeza técnica de motor e rodas", "Proteção de pintura com cera", "Condicionamento de plásticos", "Selante protetor"]'::jsonb
);

-- 6.4 SERVIÇOS ADICIONAIS (Seção 8)
INSERT INTO public.service_catalog (name, vehicle_type, commercial_category, service_type, base_price, description, included_items)
VALUES 
(
    'Cristalização de Para-brisa', 'car', 'Todos', 'adicional', 60.00,
...
(
    'Remoção de chuva ácida dos vidros', 'car', 'Todos', 'adicional', 80.00,
...
(
    'Higienização de bancos de tecido', 'car', 'Todos', 'adicional', 150.00,
